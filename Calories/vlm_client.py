...
# ── Calories 專用:串流 + JSON 陣列閉合即停(把 35 秒縮到 10 幾秒)──────────────
def ask_json_stream(frame_bgr, question, system_prompt=None, temperature=0.15, max_tokens=220, on_piece=None):
    """
    和 ask_about_image 相同輸入,但用 vlm.generate() 逐 token 收,
    偵測到最外層 JSON 陣列 `[ ... ]` 閉合就提前結束,不等模型把解釋文字講完。
    回傳 (text, elapsed_seconds)。
    """
    vlm, err = _ensure_loaded()
    if err:
        return err, 0.0
    try:
        ...
        return f"[vlm_client] 影像前處理失敗：{e}", 0.0
    sys_txt = system_prompt or DEFAULT_SYSTEM_PROMPT
    prompt = [
    ...
    try:
        ...
        pass
    t0 = time.time(); out = ""; depth = 0; opened = False
    try:
        with vlm.generate(prompt=prompt, frames=[img], temperature=temperature, seed=DEFAULT_SEED,
                          max_generated_tokens=max_tokens, frequency_penalty=DEFAULT_FREQ_PENALTY,
                          top_p=DEFAULT_TOP_P, top_k=DEFAULT_TOP_K, do_sample=True) as gen:
            for tok in gen:
                piece = tok if isinstance(tok, str) else str(tok)
                out += piece
                if on_piece:
                    on_piece(piece)
                for ch in piece:
        ...
        traceback.print_exc()
        return f"[vlm_client] 推論失敗：{type(e).__name__}: {e}", time.time() - t0
    dt = time.time() - t0
    clean = _clean_response(out)
    print(f"[vlm_client] 串流推論完成（{dt:.1f}s，{len(out)} chars）", flush=True)
    return clean, dt
