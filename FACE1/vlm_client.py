...
def close():
    """主動釋放 VDevice + VLM，讓後續其他 Hailo 程式（LLM）接手。"""
    _release()
...
# ── 串流版（逐字輸出，讓畫面在推論時「會動」） ──────────────────────────
# 重點：generate_all 是一個會卡住整個 Python（GIL）的原生呼叫，所以「另開執行緒
# 印轉圈圈」在它跑的時候根本排不進去（這就是 WQ1/V2 動畫一直沒出來的原因）。
# 改用逐字串流後，模型每吐一個 token 就回到 Python 印一次，畫面自然會動。
#
# 本函式「執行期自動偵測」串流 API：先試 vlm.generate(...) 取得可迭代的 token 串流；
# 若該版 HailoRT 沒有這個方法或參數對不上，就自動退回 generate_all（你已跑通的路），
# 最壞情況等同舊行為，不會讓 demo 崩。ask_about_image 完全沒動，V1/V3 不受影響。
_STREAM_CUTS = ("<|im_end|>", "<|endoftext|>", ". [{'type'")


def ask_about_image_stream(frame_bgr, question, on_token=None,
                           system_prompt=None,
                           temperature=DEFAULT_TEMPERATURE,
                           seed=DEFAULT_SEED,
                           max_tokens=DEFAULT_MAX_TOKENS,
                           ...
                           top_k=DEFAULT_TOP_K):
    """
    與 ask_about_image 相同的輸入，但會「逐字」把 token 透過 on_token(piece) 回拋，
    讓呼叫端可以即時印出來（畫面會動）。回傳清乾淨的完整字串。
      on_token  = callable(str)，每收到一段文字就被呼叫一次；給 None 則不回拋。
    """
    def _emit(piece):
    ...
    # ① 先試逐字串流
    try:
        stream = vlm.generate(**gen_kwargs)  # 預期回傳可迭代的 token 串流
        for piece in stream:
            streamed_any = True
            ...
            # 偵測到結束標記就只印到標記前、然後停
            cut_at = None
            for c in _STREAM_CUTS:
                i = combined.find(c)
                if i != -1:
                    cut_at = i if cut_at is None else min(cut_at, i)
            if cut_at is not None:
                visible = combined[:cut_at]
                _emit(visible[len(acc):])
                acc = visible
                break
            _emit(piece)
            acc = combined
        clean = _clean_response(acc)
        print(f"[vlm_client] 串流完成（{time.time()-t0:.1f}s，{len(clean)} chars）",
              flush=True)
        return clean
    except Exception as e:
        # 串流到一半才壞 → 別再重跑，回傳已收到的，避免重複輸出
        if streamed_any:
            print(f"[vlm_client] 串流中斷，用已收到內容：{e}", flush=True)
            return _clean_response(acc)
        # 這版沒有 generate / 參數對不上 → 退回 generate_all（舊的可靠路）
        print(f"[vlm_client] 無串流 API（{type(e).__name__}），退回 generate_all。",
              flush=True)

    # ② 退路：一次性 generate_all（不會逐字動，但保證有結果）
    try:
        resp = vlm.generate_all(**gen_kwargs)
        clean = _clean_response(resp if isinstance(resp, str) else str(resp))
        _emit(clean)  # 一次性整段回拋
        print(f"[vlm_client] generate_all 完成（{time.time()-t0:.1f}s，{len(clean)} chars）",
              flush=True)
        return clean
    except Exception as e:
        traceback.print_exc()
        msg = (f"[vlm_client] 推論失敗：{type(e).__name__}: {e}\n"
               f"請確認 .hef 與 HailoRT 5.3.x 相容，以及 UGen300 沒有被其他程式佔用。")
        _emit(msg)
        return msg
